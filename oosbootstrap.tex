...
that add results on the rates of convergence.
...
$L_2$-mixingale of size $-1/2$; moreover, $h_t/t^{1-a-\delta}$ is also
an $L_2$-mixingale of size $-1/2$ with constants $c_t/t^{1-a-\delta}$
where $c_t$ and $\zeta_j$ denote its mixingale constants and
coefficients.
...
The previous results establish that both $\bh_t^*$ and $\btrue^*$
