...
Now, for any $t = R,\dots,T-1$,
\begin{align}\label{eq:18}
  \pr\Big[\big\lvert \tfrac{1}{t} \sum_{s=1}^t & \nabla^2 q_s^*(b_t^*) - B^{*-1} \big\rvert > \epsilon \Big] \\
  &\leq \pr\Big[\big\lvert \tfrac{1}{t} \sum_{s=1}^t \nabla^2 q_s^* - B^{*-1} \big\rvert 1\{\beta_0^* \in N\}> \epsilon \Big]\\
  &\quad + \pr\Big[\big\lvert \tfrac{1}{t} \sum_{s=1}^t (\nabla^2 q_s^*(b_t^*) - \nabla^2 q_s^*) \big\rvert 1\{\beta_0^* \in N,\ \bh_t^* \in N\} > \epsilon \Big] \\
  &\quad + \pr[\beta_0^* \notin N] + \pr[\bh_t^* \notin N]
\end{align}
As before, the last two probabilities converges to zero since
$\btrue^*$ is consistent for $\btrue$ and $\bh_t^*$ is consistent for
$\btrue^*$. Moreover, just as in the proof of Theorem~\ref{res:3},
$(1/t) \sum_{s=1}^t \nabla^2 q_s(\btrue^*)$ can be reexpressed as the
sum of a uniformly integrable \mds\ that obeys the \lln, so the first
probability on the rhs of~\eqref{eq:18} converges to zero. Finally,
since $\nabla^2 q_s$ is continuous uniformly in $N$, we can choose
$\delta$ so that $|\beta_1 - \beta_2| < \delta$ implies that
$|\nabla^2 q_s(\beta_1) - \nabla^2 q_s(\beta_2)| < \epsilon$. Then
\begin{multline*}
  \pr\Big[\big\lvert \tfrac{1}{t} \sum_{s=1}^t (\nabla^2 q_s^*(b_t^*) - \nabla^2 q_s^*) \big\rvert 1\{\beta_0^* \in N,\ \bh_t^* \in N\} > \epsilon \Big]
  \leq \\
\pr[ \lvert b_t^* - \btrue^* \rvert > \delta\text{ and } \btrue^* \in N,\text{ and } \bh_t^* \in N]
\end{multline*}
which again converges to zero by consistency of $\btrue^*$ and $\bh_t^*$.
...
\subsection{Probably needs to be removed}
