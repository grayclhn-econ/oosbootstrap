...
\begin{equation}\label{eq:9}
  \hat\beta_t = \begin{cases}
    \argmin_\beta \tfrac1{t-1} \sum_{s=1}^{t-1} q(y_{s+1}, x_s, \beta) & \text{recursive window} \\
    \argmin_\beta \tfrac1{R-1} \sum_{s=t-R+1}^{t-1} q(y_{s+1}, x_s, \beta) & \text{rolling window} \\
    \argmin_\beta \tfrac1{R-1} \sum_{s=1}^{R-1} q(y_{s+1}, x_s, \beta) & \text{fixed window},
  \end{cases}
\end{equation}
...
confidence intervals, etc. And, after spelling out our specific
assumptions, that's what we'll show!
...
First result is proving that the bootstrap variance estimator is consistent.
\begin{thm}\label{res:1}
  Under Assumptions~\ref{a1} -- \ref{a7}, $\var(\fb)/\var^*(\fb^*) \to^p 1$.
\end{thm}

Then the main result is proving asymptotic normality.
