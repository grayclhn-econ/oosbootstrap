...
\begin{lema}\label{res:a2}
  ...
  \begin{gather}
    ...
    \omax{t} t^a | \hat{\beta}_{t} - \beta_{0} | \to^{p} 0 \label{eq:12}
    \intertext{and}
    \omax{t} t^a | \hat{\beta}^{*}_{t} - \hat{\beta}_T | \to^{p^{*}} 0. \label{eq:13}
  \end{gather}
\end{lema}
...
Then let $\delta$ be a positive number less than $1/2 - a$ and define
...
\begin{gather}
  ...
  \Big[\sum_{t = 1}^{\ell_{i}} (h_{u + t}(\btrue^*) - h_{u + t})/ (K_{i-1} + t)^{1-a-\delta}\Big]^2  = O_p(1)
  \label{eq:5}
  \intertext{and}
  \omax{t} \big| \sum_{s=K_{j^*_{t - 1}}+1}^{t-1} h_s^*/(t-1)^{1-a-\delta} \big| = O_{p^*}(1).
\end{gather}

First we'll prove~\eqref{eq:4}. As in \citet{Cal:15}, our assumptions
ensure that $h_t$ is an $L_2$-mixingale of size $-1/2$. If $c_t$ and
$\zeta_j$ denote its mixingale constants and coefficients,
$h_t/t^{1-a-\delta}$ is also an $L_2$-mixingale of size $-1/2$ with
constants $c_t/t^{1-a-\delta}$. Then \citepos{Mcl:75} maximal
...
\begin{align*}
  ...
  &\leq O(1) \sum_{i=1}^J \ell^2 / T K_{i-1}^{2-2a-2\delta}
\end{align*}

\begin{align*}
  &\leq \pr\Big[\tfrac{P^{2a}}{T-1} \sum_{u = 0}^{T-2} \sum_{i=1}^J \Big(\sum_{t = 1}^{\ell}
  (h_{u + t}(\btrue^*) - h_{u + t}(\btrue))/ (K_{i-1} + t)\Big)^2 1\{\beta_0^* \in N\} > \epsilon^2 \Big] \\
  &\quad + \pr[\beta_0^* \notin N] \\
  &\leq \epsilon^{-2} \tfrac{P^{2a}}{T-1} \sum_{u = 0}^{T-2}  \sum_{i=1}^J \E \sup_{\beta \in N} \Big(\sum_{t = 1}^{\ell}
  \nabla h_{u + t}(\beta) \cdot (\btrue^* - \btrue)/ (K_{i-1} + t)\Big)^2 + o(1) \\
  &\leq O(1/T^2) \sum_{u = 0}^{T-2} \sum_{i=1}^J \E \Big(\sum_{t = 1}^{\ell} m_{u + t} / (K_{i-1} + t)\Big)^2 + o(1) \\
  &\leq O(P^{2a}/T) \sum_{t = 1}^{T-1}
  \ell c^2/t^2 + o(1)
\end{align*}
and (need $m_t$ to be NED)
\begin{align*}
  \E \Bigg| (P^{2a}/T) &\tfrac{1}{T-1} \sum_{u = 0}^{T-2} \sum_{i=1}^J
  \Bigg[\sum_{t = 1}^{\ell_{i}} \lVert \nabla \tilde{h}_{u + t}(\tilde \beta_T)
  \rVert / (K_{i-1} + t)\Bigg]^2 \Bigg| \\
  &= (P^{2a}/T) \tfrac{1}{T-1} \sum_{u = 0}^{T-2} \E \sum_{i=1}^J
  \E \Bigg(\Bigg[\sum_{t = 1}^{\ell_{i}} \lVert \nabla \tilde{h}_{u + t}(\tilde \beta_T)
...
\begin{proof}[Proof of~\eqref{eq:12} and~\eqref{eq:13}.]
We have
\begin{multline}
  P^a \sup_t | \hat{\beta}_t - \btrue | = P^a \sup_t |\hat{B}_{t} H_{t}| \\
  \leq \sup_{t,u} \Big| [ \hat{B}_u - B]
...
previous argument and by assumption.  The same argument holds for
$\hat{\beta}_t^{*} - \btrue^*$ as well.
\end{proof}
