...
We will only present proofs for the recursive window and the circular
block bootstrap. The other methods have very similar proofs.  To make
the mathematical results in this appendix clearer, we will introduce
...
\newcommand{\WesA}[1][]{\oclt{t}
  (F_t^{#1} - F^{#1}) B^{#1} \tfrac{1}{t-1} \sum_{s=1}^{t-1} h_s^{#1}}
...
\begin{equation}\label{eq:23}
  \pr^*[\sqrt{P} (\fb^* - \E^* f_t^*) / \var^*(\sqrt{P} \bar f^*)^{1/2} \leq x]
  \to^p \Phi(x)
\end{equation}
and
...
$f_t^{*}(\bh_t^*)$ around $\btrue^*$ to get
  ...
  \begin{equation}\label{eq:19}
    \begin{pmatrix}
      ...
      \lambda_{fh} S_{fh}^{*\prime} &  \lambda_{hh} S_{hh}^*
    \end{pmatrix}^{-1/2}
    ...
    N(0, I)
  \end{equation}
  which implies~\eqref{eq:23}. Moreover,
  \begin{equation*}
    \var^*(\sqrt{P} \bar{f}^*) = S_{ff}^* +
    \lambda_{fh} (F^* B^* S^*_{fh} + S_{fh}^{*\prime} B^{*\prime}F^{*\prime}) +
    \lambda_{hh} F^* B^* S^*_{hh} B^{*\prime} F^{*\prime}.
  \end{equation*}
  and converges to $\sigma^2$ in probability by Lemmas~\ref{SA1}
  and~\ref{SA5}, completing the proof.\qed
...
\begin{lema}\label{SA4}
  ...
  \begin{gather}
    \label{eq:18}
    \oclt{t} (F_t^* - F^*) B^* \sum_{s=1}^{t-1} h_s^* / (t-1) \to^{p^*} 0 \\
    \label{eq:17}
    \oclt{t} F^* (B_t^* - B^*) \sum_{s=1}^{t-1} h_s^* / (t-1) \to^{p^*} 0 \\
    \label{eq:25}
    \oclt{t} (F_t^* - F^*) (B_t^* - B^*) \sum_{s=1}^{t-1} h_s^* / (t-1) \to^{p^*} 0
  \end{gather}
  and
  \begin{equation}\label{eq:16}
    \sup_{b_{it} \in \rect(\btrue^*, \bh_t^*)} \oclt{t}
...
\begin{align}\label{eq:A3}
  &\WesA[*] \notag \\
  &\qquad= \WesA[*] \Ind\big\{ \big\lvert (t - 1)^{a-1} B^*
  \sum\nolimits_{s=1}^{t-1} h_s^* \big\rvert > \delta\big\} \notag \\
  &\quad\qquad + \WesA[*] \Ind\big\{ \big\lvert (t - 1)^{a-1} B^*
  \sum\nolimits_{s=1}^{t-1} h_s^* \big\rvert \leq \delta \big\}.
\end{align}
For any $c > 0$,
\begin{multline*}
  \pr^*\Bigg[ \Big\lvert \WesA[*] \Ind\big\{ \big\lvert (t - 1)^{a-1} B^*
  \sum\nolimits_{s=1}^{t-1} h_s^* \big\rvert > \delta\big\}
  \Big\rvert > c \Bigg] \\
  \leq \pr^*\Big[\omax{t} \big\lvert (t - 1)^{a-1} B^*
  \sum\nolimits_{s=1}^{t-1} h_s^* \big\rvert > \delta \Big]
\end{multline*}
which converges to zero in probability by Lemma~\ref{SA3}, so the
first summation on the RHS of~\eqref{eq:A3} converges to zero in
probability as well. The second summation is of order
\[
  \oclt{t} (F_t^* - F^*) / (t-1)^a
\]
and converges in probability to zero by Lemma~\ref{SA3}.
\end{proof}

\begin{proof}[Proof of~\eqref{eq:17}]
  We have
  \[
    \big\lvert \oclt{t} F^* (B_t^* - B^*) \sum_{s=1}^{t-1} h_s^* / (t-1) \big\rvert
    = O_{p^*}\big(\omax{t} \lvert B_t^* - B^* \rvert\big)
    \oclt{t} \Big\lvert \tfrac{1}{t-1} \sum_{s=1}^{t-1} h_s^* \Big\rvert,
  \]
  so the conclusion follows from Lemmas~\ref{SA2} and~\ref{SA3}.
\end{proof}

\begin{proof}[Proof of~\eqref{eq:25}]
  Here
  \begin{multline*}
    \oclt{t} (F_t^* - F^*) (B_t^* - B^*) \sum_{s=1}^{t-1} h_s^* / (t-1) = \\
    O_p\big(\omax{t}\lvert B_t^* - B^* \rvert \big)
    \Big(\oclt{t} (F_t^* - F^*)  \sum_{s=1}^{t-1} h_s^* / (t-1) \Big).
  \end{multline*}
  The first term is $o_p(1)$ from Lemma~\ref{SA2} and the second is $o_p(1)$ from
  an argument essentially identical to that of~\eqref{eq:18}.
\end{proof}
...
Lemmas~\ref{SA1} and~\ref{SA3} and the random variable inside the
first probability has the upper bound
\begin{multline*}
  \big\lvert 1\{\btrue^* \in N, \bh_t^* \in N\ \text{for all}\ t\}
  \oclt{t} (\bh_t^* - \btrue^*)' \nabla^2 f_{it}^*(b_{it}) (\bh_t^* - \btrue^*) \big\rvert
  \\ \leq
  \big(\sup_{t=R,\dots,T-1} |P^{1/4}(\bh_t^* - \btrue^*)|^2\big) \oavg{t}
  \lvert \nabla^2 f_{it}^*(b_{it}) \rvert 1\{\btrue^* \in N, \bh_t^* \in N\}.
\end{multline*}
The summation is $O_p(1)$ by assumption and the supremum term converges to zero
by Lemma~\ref{SA3}.
\end{proof}

\begin{lema}\label{SA5}
  Under the conditions of Theorem~\ref{T1}, $S_{ff}^* \to^p S_{ff}$,
  $S_{fh}^* \to^p S_{fh}$, and $S_{hh}^* \to^p S_{hh}$.
\end{lema}
\begin{proof}
  The result holds from rewriting the variances under the bootstrap
  distribution a HAC estimators of the population variance, then
  applying the results of \citet{JoD:00}.
\end{proof}
...
\begin{proof}
  ...
  \begin{equation*}
    \sum_{j=1}^J z_j^* / v^* \to^d N(0, 1)
  \end{equation*}
  as long as the following properties hold (along with Lemma~\ref{SA5}):
  ...
  \begin{equation*}
   ...
   \end{equation*}
   Uniform integrability of the summand follows from similar arguments
   to those used in \cite{Cal:14}, making this term is the sum of a
   uniformly integrable martingale difference sequence and satisfies
   the \lln\ (i.e.\ Davidson's, 1994, Theorem 19.7), so it converges
   in (conditional) probability to zero. The second term converges in
   probability to zero by consistency of $\btrue^*$ (Lemma~\ref{SA1}).

   Similarly, \eqref{eq:7} holds if
   \begin{equation*}
     1\{\beta_0^* \in N\} \sum_{j=1}^J
     \E^*(z_j^{*2} 1\{z_j^{*2} > \epsilon\} \mid \Hs_{j-1}^*) \to^p 0,
   \end{equation*}
   which again follows from uniform integrability.  The formula for
   the limiting variance follows from West (1996), completing the
   proof.
\end{proof}
