...
(which we will define shortly) --- and then
...
paper presents results for $M$-estimators, like \citepos{CoS:07}, the
...
the statistics that we cover in this paper. Section~\ref{sec:3}
presents an empirical illustration of our approach based on
\citepos{Cal:15} mixed-window \oos\ statistic, and Section~\ref{sec:mc} presents a
...
misapplication of \citepos{Han:05} \spa\ algorithm, that bootstraps
the $\fh_{it}$ values to generate a critical value. This is the
procedure proposed by Hansen, but for a very different setting where
$R$ is large and $P$ is small.

Two predictors are significant at the naive and the incorrect \spa\
critical values: the long term interest rate and the book to market
ratio. But none are significant after correcting for both parameter
estimation error and data snooping with our proposed approach.
